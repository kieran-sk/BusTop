...
class LiveTrackingService : Service() {
 ...
 // Poll every 20 seconds for accurate live updates
 delay(20000)
 }
 }
 }
 ...
 return try {
 val url = URL(https://bustop.pages.dev/api/stops//arrivals)
 ...
 val title = if (mins <= 0) 🚨 Γραμμή  • ΕΦΤΑΣΕ ΣΤΗ ΣΤΑΣΗ! else 🚍 Γραμμή  • σε  λεπτά
 ...
 .setOngoing(true) // Keeps it fixed in the status bar & notification shade
 ...
 // Start continuous ringing foreground service until dismissed
